...
struct ContentView: View {
    @StateObject private var parser = MLXParser()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            switch parser.modelState {
            case .downloading(let fraction, let detail):
                downloadingView(fraction: fraction, detail: detail)
            case .loading:
                loadingModelView
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("ControlPlane — Natural Language Config (MLX)")
                    .font(.headline)
                Text("On-device Qwen3 4B via mlx-swift-lm (issue #536)")
                    .font(.caption)
    ...
    // MARK: - Pre-ready states

    private func downloadingView(fraction: Double, detail: String) -> some View {
        VStack(spacing: 20) {
            Spacer()
            ...
            Text("Downloading Qwen3 4B")
                .font(.title2)
                .fontWeight(.medium)
            Text("~2.8 GB — this only happens once. The model is cached locally.")
                .foregroundStyle(.secondary)
    ...
    private var readyView: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                examplesSection
                if parser.isLoading {
                    inferenceLoadingView
                } else if let result = parser.result {
                    resultView(result)
                }
                if let err = parser.error {
    ...
    private var inputSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                ...
                .disabled(input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || parser.isLoading)
    ...
    private var inferenceLoadingView: some View {
        HStack(spacing: 12) {
            ProgressView()
    ...
    private func resultView(_ config: ParsedConfig) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            SectionBox(title: "Profile", icon: "person.crop.rectangle") {
                ...
                KeyValueRow(key: "Create new", value: config.createProfile ? "Yes" : "No — assumed to exist")
            }
...
// MARK: - Sub-views (shared with original NLConfig harness)
...
struct RuleView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                Text(rule.name.isEmpty ? "Rule" : rule.name).fontWeight(.medium)
                if rule.negate {
                    Text("NEGATED")
                        .font(.caption2)
            ...
            HStack(spacing: 6) {
                ...
                Text(rule.readingKey).font(.caption).foregroundStyle(.secondary)
                Text(rule.operatorID).font(.caption).foregroundStyle(.purple)
                Text("\"\(rule.comparandValue)\"").font(.caption)
            }
        }
        .font(.callout)
    }
...
struct ActionView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                Text(action.actionID).fontWeight(.medium).font(.callout)
                Spacer()
                Text(action.trigger)
                    .font(.caption)
                    .padding(.horizontal, 6).padding(.vertical, 2)
